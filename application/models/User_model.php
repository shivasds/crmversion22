...
class User_model extends CI_Model {
    ...
    public function add_user($data){
        ...
        if((in_array($data['type'], array(1,2))) && ($data['email'])){
            ...
            $this->email->subject("Welcome to Prop Solution");
            $this->email->message("Welcome to Prop Solution CRM System,<br><br>Your user name is ".$data['emp_code']." And password is ".$data['emp_code']." by using them please login to the tool with the following link: <a href=\"http://crm.propsolutionservices.com/login\" >http://crm.propsolutionservices.com/login</a> <br><br>Regards Prop Solution IT team");
            $this->email->send();
        }
    }
    ...
    function getUsersEmails(){
        ...
        $exceptions = array("ranjeet.singh@propsolutionservices.com",);
        $emails = array();
        foreach ($users as $key => $value) {
          if($value->email)
            if(!in_array($value->email,$exceptions))
